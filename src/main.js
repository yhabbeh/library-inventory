...
import { fetchBooksData, processCategories, normalizeAvailability } from './data.js';

// Removed SHEET_URL as it is now in data.js
...
async function fetchBooks() {
  try {
    ...
    const catData = processCategories(books, allLabel);

    categories = catData.categories;
    subCategoriesMap = catData.subCategoriesMap;
    categoryHierarchy = catData.categoryHierarchy;
...
// normalizeAvailability imported from data.js

// parseCSV, parseCSVLine, and formatCategory imported from data.js

function getCategoryColor(category) {
  return '3f3f46';
}

// processCategories imported from data.js
...
window.setLanguage = (lang) => {
  ...
  // allLabel already defined above
  currentCategory = allLabel;
  currentSubCategory = null;
